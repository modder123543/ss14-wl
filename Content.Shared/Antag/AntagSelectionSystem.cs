...
/// </remarks>
public abstract partial class AntagSelectionSystem : GameRuleSystem<AntagSelectionComponent>
{
    ...
    [Dependency] protected FollowerSystem Follower = default!;
    [Dependency] protected LoadoutSystem Loadout = default!;
    [Dependency] protected SharedAudioSystem Audio = default!;
    [Dependency] protected SharedJobSystem Jobs = default!;
    ...
    protected override void Started(Entity<AntagSelectionComponent, GameRuleComponent> rule, ref GameRuleStartedEvent args)
    {
        ...
        if (rule.Comp1.SelectionTime == RuleStarted) // Only pre-select antags if we pre-select on rule start
            AssignAntags(rule, players);
        else // Otherwise, we only spawn the ghost roles!
            SpawnGhostRoles(rule, players.Length);
    }
    ...
    private bool PreAssignAntag(ICommonSession player, ref List<AntagRule> antags)
    {
        ...
        for (var i = antags.Count - 1; i >= 0; i--)
        {
            ...
            {
                DebugTools.Assert(antag.Definition.SpawnerPrototype != null,
    ...
    protected void PreSelectSession(Entity<AntagSelectionComponent> gameRule, ProtoId<AntagSpecifierPrototype> protoId, ICommonSession player)
    {
        ...
        AdminLogger.Add(LogType.AntagSelection, $"Pre-selected {player.Name} as antagonist: {ToPrettyString(gameRule)}, {protoId}");
    }
        ...
        HashSet<ICommonSession> set)
    {
        ...
        AdminLogger.Add(LogType.AntagSelection, $"De-selected {player.Name} as antagonist: {ToPrettyString(gameRule)}, {protoId}");
    }
        ...
        ICommonSession player)
    {
        ...
        XForm.SetMapCoordinates((uid, xform), coordinates.Value);
        return uid;
    }
    ...
    /// </summary>
    protected bool TrySpawnAntagonist(Entity<AntagSelectionComponent> gameRule,
        AntagSpecifierPrototype prototype,
        ICommonSession player,
    ...
    private void AssignPreSelectedSessions(Entity<AntagSelectionComponent> gameRule)
    {
        foreach (var (proto, set) in gameRule.Comp.PreSelectedSessions)
        {
            ...
            {
                AdminLogger.Add(LogType.AntagSelection, $"Start trying to make {session} become the antagonist: {ToPrettyString(gameRule)}, {proto}");
    ...
    /// </summary>
    protected void InitializeAntag(Entity<AntagSelectionComponent> gameRule, AntagSpecifierPrototype prototype, EntityUid antag, ICommonSession player)
    {
        // Make sure player was properly pre-selected.
        DebugTools.Assert(gameRule.Comp.PreSelectedSessions.TryGetValue(prototype.ID, out var value) && value.Contains(player),
        ...
        Loadout.Equip(antag, gear, prototype.RoleLoadout);

        // Ensure that we have the right mind for our entity.
        if (!Mind.TryGetMind(player, out var mind, out var mindComp) || mindComp.OwnedEntity != antag)
            mind = Mind.CreateMind(player.UserId, Name(antag));

        Mind.TransferTo(mind, antag, ghostCheckOverride: true);
        Role.MindAddRoles(mind, prototype.MindRoles, silent: true);
        ...
        AdminLogger.Add(LogType.AntagSelection, $"Assigned {ToPrettyString(antag):target}, mind {ToPrettyString(mind):target} as antagonist: {ToPrettyString(gameRule):user}");
    ...
    protected void AssignMind(Entity<AntagSelectionComponent> gameRule, ProtoId<AntagSpecifierPrototype> proto, EntityUid mind, EntityUid antag)
    {
